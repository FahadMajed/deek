...
class Validator {
  static String? stringValidator(
    ...
    title = title.tr;
    if (stringToValidate!.isEmpty) return "$title ${"Cannot Be Empty".tr}";

    if (stringToValidate.length < minChar) {
      return "$title ${"must be at least".tr} $minChar ${"characters long".tr}";
    }

    if (stringToValidate.length > maxChar) {
      return "$title ${"must be less than".tr} $maxChar ${"characters long".tr}";
    }
